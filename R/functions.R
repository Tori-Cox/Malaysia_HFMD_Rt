...
plot_univariable_beta <- function(univar_results_table, choice = "all"){
  ...
  res$group <- c("epi", "hols", rep(c("rain", "max_hum","mean_hum", "min_hum",
                                      "max_temp", "mean_temp", "min_temp"), times=8),
                 rep("hols", times=7)) 
  ...
  res$group2<- substrRight(as.character(res$var),1)
  res$group2[1:9]<-0
  ...
  if(choice=="only_significant"){
    ...
    p<-res%>% 
      mutate(group=sub("hols", "Proportion of school holiday days", group),
             ...
             group=sub("epi", "Proportion of serotype EV-A71", group))%>%
...
run_multivar_analysis <- function(data, variables_to_try, variables_chosen_already = NA, stage = NA){
  ...
  for(i in 1:length(var_names)){
    ...
    formula <- logRt ~ 1 + cum_inc + data[,variables_chosen_already] +
      f(State, model="iid", hyper=precision.prior)+
      f(YEAR_NO, model="rw1", replicate=ISLAND_NO, hyper=precision.prior)+
      data[,var_names[i]]
    }
    if(length(variables_chosen_already)==2){
      formula <- logRt ~ 1 + cum_inc + data[,variables_chosen_already[1]] + data[,variables_chosen_already[2]] +
        f(State, model="iid", hyper=precision.prior)+
        f(YEAR_NO, model="rw1", replicate=ISLAND_NO, hyper=precision.prior)+
        data[,var_names[i]]
    }
    if(length(variables_chosen_already)==3){
      formula <- logRt ~ 1 + cum_inc + data[,variables_chosen_already[1]] +
...
run_final_model <- function(data){
  ...
  formula <- logRt ~ 1 + cum_inc + prop_hols + max_temp  + max_hum + ev71  + 
    cum_rf4 +
    f(State, model="iid", hyper=precision.prior)+
    f(YEAR_NO, model="rw1", replicate=ISLAND_NO, hyper=precision.prior)
  ...
  model <- inla(formula = formula,
                data = data, family="gaussian",
                control.family=list(link="identity"),
                control.compute = list(waic = TRUE, config=T),
                control.inla=list(int.strategy = "eb"), verbose=F)
